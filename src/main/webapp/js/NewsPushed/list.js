...
function fn(res_key,resUrl){
	$('#modal_btn').show();
	if(res_key=='push_message'){
        addForm(resUrl);
	}else 
		if(res_key=='del_push_message'){
			var id = $('input[name="id"]').filter(':checked').val();	
			if(id == undefined ){
				ShowFailure('删除操作必须选中对应的记录！');
				return ;
			}
			deleteNewsPushed(resUrl = resUrl + '?id='+id);
			findPage({serviceName:serviceName});
	    	  ShowSuccess('操作成功！');
		}
}
 ...
 * @param resUrl
 */
function deleteNewsPushed(resUrl){
	$.ajax({  
	      url:resUrl,  
	      dataType: "JSON",
...
function addForm(resUrl){
    initForm({
        lable:'推送消息', 
        init_url:resUrl,
        submit_url:'NewsPushed/save.do',
        serviceName:'NewsPushed'
    });
}
...
function detailsById(id){
	 ...
	 initForm({
			lable:'推送消息详情页面',
			init_url:"NewsPushed/detailsById.do?id="+id,
			submit_url: '#',
			serviceName:'NewsPushed',
