...
function fn(res_key,resUrl){
    if(res_key=='terraceActive_add'){
        addForm(resUrl);
    }else 
		if(res_key=='terraceActive_del'){
			var id = $('input[name="id"]').filter(':checked').val();	
			if(id == undefined ){
				ShowFailure('删除操作必须选中对应的记录！');
				return ;
			}
			deleteTerraceActive(resUrl = resUrl + '?id='+id);
			findPage({serviceName:serviceName});
	    	  ShowSuccess('操作成功！');
		}else{
			fs(res_key,resUrl);
...
function addForm(resUrl){
    initForm({
        lable:'新增业务规则', 
        init_url:resUrl,
        submit_url:'terraceActive/save.do',
        serviceName:'terraceActive'
    });
}
 ...
 * @param resUrl
 */
function deleteTerraceActive(resUrl){
	$.ajax({  
	      url:resUrl,  
	      dataType: "JSON",
...
function fs(res_key,resUrl){
	...
	updateTerraceActive(resUrl = resUrl + '?id='+id);
}

//修改规则
function updateTerraceActive(resUrl){
	initForm({
		lable:'修改业务规则页面',
		init_url:resUrl,
		submit_url:'terraceActive/update.do',
		serviceName:'terraceActive'
	});
}
